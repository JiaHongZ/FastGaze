...
def run_model(model, src, task, device = "cuda:0", im_h=20, im_w=32, patch_size = 16, num_samples = 1, batch_type=0):
    ...
    if batch_type == torch.Tensor([[2]]):
        ...
        mask = 1
    with torch.no_grad():
        out_task, token_prob, ys, xs, ts = model(src = src, tgt = firstfix, task = task * mask, type_tensor=batch_type)
...
def test_FV(args,model):
    ...
    t_dict = {}
    for traj in test_target_trajs:
        key = 'test-{}-{}-{}'.format(traj['condition'],
                                     traj['name'][:-4], traj['subject'])
...
def main(args):
    ...
    seq_score, ed = test_FV(args,model)
    output_str2 = f"Epoch: {epoch}, FV: seq_score: {seq_score:.3f}, ed: {ed:.3f}\n"
    print(output_str2)
