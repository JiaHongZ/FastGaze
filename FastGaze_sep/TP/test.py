...
def test(args,model):
    ...
    if test_saliency_map:
        for traj in predictions:
            ...
            # 320,512 是对的
                # 25 nss: 8.35701964205035 cAUC: 0.901308809074701  gazeformer nss: 8.500036932942042 cAUC: 0.9019553156470854
                # 35 nss: 7.5863622763856275 cAUC: 0.9126869714608552 gazeformer nss: 7.675439713931235 cAUC: 0.9125545169154456
                # 15 nss: 9.342417354920297 cAUC: 0.849444851561699 gazeformer nss: 9.528332554625067 cAUC: 0.85179029123326
                # 10 - 9， 30- 7.81  20 - 8.901756900902061 cAUC: 0.8851985071825653
...
def main(args):
    ...
    model = fastgaze(vs, spatial_dim = (args.im_h, args.im_w), max_len = args.max_len, device = device)
    ...
    output_str = f"seq_score: {seq_score:.3f}, seq_score_t: {seq_score_t:.3f}, sem_seq_score: {sem_seq_score:.3f}, sem_seq_score_t: {sem_seq_score_t:.3f}, ed: {ed:.3f}, ed_t: {ed_t:.3f}, sem_ed: {sem_ed:.3f}, sem_ed_t: {sem_ed_t:.3f}\n"
    print(output_str)

    
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Gazeformer Train', parents=[get_args_parser_test()])
