...
def plot_scanpath(img, xs, ys, ts, bbox=None, title=None, cir_rad_min=15, cir_rad_max=50):
    fig, ax = plt.subplots()
    ax.imshow(img)
    ...
    if title is not None:
        ax.set_title(title)
    plt.savefig('scanpath'+'_'+args.trained_model+'_'+args.task+'_'+args.imgfile+'.png')
    # plt.show()
...
if __name__ == '__main__':
    ...
    T = T * width_scale
    plot_scanpath(img_resized, X, Y, T, new_bbox, title, cir_rad_min=5, cir_rad_max=8)
