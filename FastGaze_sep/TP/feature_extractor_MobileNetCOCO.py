from models import MobileNetCOCO
...
def image_data(dataset_path, device = 'cuda:0', overwrite = False):
    ...
    target_path = join(dataset_path, 'image_features_MobileNetCOCO/')
    resize = T.Resize(resize_dim)
    normalize = T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
    folders = [i for i in os.listdir(src_path) if isdir(join(src_path, i))]

    bbone = MobileNetCOCO(device = device).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_path', default= '/zjh/data/COCO-Search18andCOCO-Freeview', type=str)
