...
def compute_mm(human_trajs, model_trajs, im_w, im_h, tasks=None):
    ...
    for traj in model_trajs:
        ...
        all_mm_scores.append((task,
                              np.mean([
                                  multimatch(traj, gt_traj, (im_w, im_h))
...
def generate_gaussian_map(x, y, sigma_x=30, sigma_y=30, H=320, W=512):
    # 创建网格
    xx, yy = np.meshgrid(np.arange(W), np.arange(H))
    # 高斯分布
    gaussian_map = np.exp(-((xx - x)**2 / (2 * sigma_x**2) + (yy - y)**2 / (2 * sigma_y**2)))
    return gaussian_map

def resize_bbox(bbox, src_size, dst_size):
    """
    调整边界框坐标从 src_size 到 dst_size 的分辨率
    :param bbox: 边界框 [x, y, width, height]
    :param src_size: 原始分辨率 (width, height)
    :param dst_size: 目标分辨率 (width, height)
    :return: 调整后的边界框 [x', y', width', height']
    """
    ...
    w_resized = w * scale_x
    h_resized = h * scale_y
    return [x_resized, y_resized, w_resized, h_resized]

def compute_NSS(human_trajs, model_trajs):
    """
    Compute the NSS score for each model trajectory compared to human trajectories.

    Parameters:
    - human_trajs: List of human trajectories with fixation points.
    - model_trajs: List of model-generated trajectories with saliency maps.
    ...
    for traj in model_trajs:
        ...
        # Filter corresponding human trajectories
        gt_trajs = list(filter(lambda x: x['name'] == img_name and x['task'] == task, human_trajs))
        values = []

        # Convert saliency map to torch tensor
        saliency_map = torch.from_numpy(np.array(traj['saliency_map'], dtype=np.float32))
        ...
        saliency_map_norm = (saliency_map - mean) / std

        for gt_traj in gt_trajs:
            # Extract fixation points
            s2x = np.array(gt_traj['X'], dtype=np.float32)
            s2y = np.array(gt_traj['Y'], dtype=np.float32)

            # Convert fixation points to tensor
            s2x = torch.from_numpy(s2x)
            s2y = torch.from_numpy(s2y)
            gt_fixs = torch.stack((s2x, s2y), dim=1)

            # Check if fixation points are within bounds
            valid_fixs = (gt_fixs[:, 0] >= 0) & (gt_fixs[:, 0] < saliency_map.size(1)) & \
                         (gt_fixs[:, 1] >= 0) & (gt_fixs[:, 1] < saliency_map.size(0))
            if not valid_fixs.any():
                continue  # Skip if no valid fixations
            gt_fixs = gt_fixs[valid_fixs]

            # Compute NSS for valid fixations
            value = saliency_map_norm[gt_fixs[:, 1].long(), gt_fixs[:, 0].long()]
            value = value.sum().item()  # Average NSS for current trajectory
            values.append(value)
            # print(value)
        # Compute average NSS for current model trajectory
        if values:
            all_mm_scores.append(np.mean(values))

    # Return overall average NSS score
    return np.mean(all_mm_scores) if all_mm_scores else 0
def compute_cAUC(human_trajs, model_trajs):
    """
    Compute cumulative AUC (cAUC) for model-generated saliency maps compared to human fixations.
    """
    import torch
    all_mm_scores = []
    ...
    for traj in model_trajs:
        ...
        # Filter human trajectories
        gt_trajs = list(
            filter(lambda x: x['name'] == img_name and x['task'] == task, human_trajs)
        )
        ...
        for gt_traj in gt_trajs:
            ...
            # Convert fixation points to tensors
            s2x = torch.from_numpy(s2x)
            s2y = torch.from_numpy(s2y)
            gt_fixs = torch.stack((s2x, s2y), dim=1).long()

            # Ensure fixation points are within bounds
            valid_fixs = (gt_fixs[:, 0] >= 0) & (gt_fixs[:, 0] < saliency_map.size(1)) & \
                         (gt_fixs[:, 1] >= 0) & (gt_fixs[:, 1] < saliency_map.size(0))
            if not valid_fixs.any():
                continue  # Skip if no valid fixations
            ...
            # Compute thresholds based on saliency map at fixation points
            thresholds = saliency_map[gt_fixs[:, 1], gt_fixs[:, 0]]
            bs = len(thresholds)
            ...
            thresholds = thresholds.view(bs, 1, 1).expand(bs, saliency_map.size(0), saliency_map.size(1))

            # Compute true positive (TP) and false positive (FP) rates
            area = []
            area.append(torch.zeros(bs, 2))  # Add (0, 0) point
            temp = torch.zeros_like(saliency_map).unsqueeze(0).expand(bs, *saliency_map.shape)
            temp[saliency_map.unsqueeze(0).expand(bs, *saliency_map.shape) >= thresholds] = 1.0
            temp = temp.view(bs, -1)

            tp = torch.ones(bs)  # All fixations are true positives
            fp = (temp.sum(dim=1) - 1) / (temp.size(1) - 1)
            area.append(torch.stack([tp, fp.cpu()], dim=1))
            area.append(torch.ones(bs, 2))  # Add (1, 1) point

            # Sort FP and TP for integration
            area = torch.stack(area, dim=1)
            sorted_indices = torch.argsort(area[:, :, 1], dim=1)
            tp_sorted = torch.gather(area[:, :, 0], dim=1, index=sorted_indices)
            fp_sorted = torch.gather(area[:, :, 1], dim=1, index=sorted_indices)

            # Integrate AUC for each sample
            values = []
            for i in range(bs):
                value = torch.trapz(tp_sorted[i], fp_sorted[i])
                values.append(value.item())

            valus.append(np.mean(values))

        if valus:
            all_mm_scores.append(np.mean(valus))
