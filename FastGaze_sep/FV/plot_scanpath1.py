...
def run_model(model, src, task, device="cuda:4", im_h=20, im_w=32, patch_size=16, num_samples=1, type_tensor=torch.Tensor([[0]]), scanpaths=None):
    ...
    with torch.no_grad():
        token_prob, ys, xs, ts = model(src=src, tgt=firstfix, task=task*0)
    ...
    for i in range(num_samples):
        ys_i = [(im_h // 2) * patch_size] + list(ys[:, i, 0])[1:] # 加了个中心点
...
if __name__ == '__main__':
    ...
    for i, scanpath in enumerate(predictions):
        X, Y, T = np.array(scanpath['X'][:args.emlength]), np.array(scanpath['Y'][:args.emlength]), np.array(scanpath['T'][:args.emlength])
