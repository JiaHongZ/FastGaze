...
class fastgaze(nn.Module):
    ...
    def forward(self, src: Tensor, tgt: Tensor, task: Tensor, freeview=True):
        src = src.to(self.device)
        # print(src.shape) # [32, 640, 2048], 32*20=640
        ...
        if freeview:
            task = task * 0
        # print('task',task.mean())
        outs = self.vs(src=src, tgt=tgt_input, tgt_mask=None, tgt_key_padding_mask = None,
        task = task.to(self.device), querypos_embed = self.querypos_embed.weight.unsqueeze(1), patchpos_embed = self.patchpos_embed)
        # print(outs.shape) # 7 32 512 -- 32是b
