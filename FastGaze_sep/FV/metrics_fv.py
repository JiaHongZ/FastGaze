...
def postprocessScanpaths(trajs):
    ...
    scanpaths = []
    for traj in trajs:
        img_name, condition, subject, fixs = traj
        scanpaths.append({
            'X': fixs[:, 1],
            'Y': fixs[:, 0],
            'T': fixs[:, 2],
...
def compute_SS(preds, clusters, truncate, reduce='mean', print_clusters = False):
    ...
    for scanpath in preds:
        key = 'test-{}-{}'.format(scanpath['condition'],
                                     scanpath['name'][:-4])
        ms = clusters[key]
        strings = ms['strings']
        cluster = ms['cluster']
...
def compute_SSS_time(preds, fixations, truncate, segmentation_map_dir, reduce='mean', tempbin=50):
    ...
    for scanpath in preds:
        ...
        '''
        print(pred, strings) 
        [('53', 216.40176391601562), ('46', 151.90789794921875), ('66', 175.25320434570312), ('66', 209.83740234375), ('66', 228.5922088623047)] [[('66', 181), ('66', 105), ('66', 410)], [('53', 251), ('46', 127), ('66', 124), ('66', 393)], [('46', 264), ('66', 76), ('46', 70), ('66', 165), ('66', 281)], [('53', 307), ('64', 80), ('66', 124), ('66', 87), ('66', 233)], [('53', 48), ('66', 150), ('66', 90), ('66', 115), ('66', 573)], [('53', 87), ('63', 136), ('46', 203), ('66', 145), ('66', 236)], [('66', 252), ('66', 137), ('66', 130), ('66', 213)], [('53', 294), ('53', 68), ('46', 242), ('46', 66), ('66', 156)], [('53', 260), ('46', 212), ('46', 21), ('66', 141), ('66', 269)], [('53', 272), ('66', 135), ('66', 106), ('66', 174)]]
        '''
        pred = pred[:truncate] if len(pred) > truncate else pred
        for p in pred:
