...
def run_model(model, src, task, device = "cuda:0", im_h=20, im_w=32, patch_size = 16, num_samples = 1):
    ...
    task = torch.tensor(task.astype(np.float32)).to(device).unsqueeze(0).repeat(num_samples, 1) * 0
...
def test(args,model):
    from metrics_fv import get_seq_score, get_semantic_seq_score, get_ed, get_semantic_ed
    
    img_ftrs_dir = ['/zjh/data/FastGaze-cocosearch/image_features_TP','/zjh/data/FastGaze-cocosearch/image_features','/zjh/data/FastGaze-cocosearch/image_features']
    ...
    if args.condition == 'freeview':
        ...
        with open('/zjh/data/Gazeformer-CVPR-2023/SemSS/test_TA_Sem.pkl', "rb") as r:
            fixations_dict = pickle.load(r)
            r.close()
    ...
    # sem_seq_score_t = get_semantic_seq_score_time(predictions, fixations_dict, max_len, '/zjh/data/Gazeformer-CVPR-2023/SemSS/stuffthing_maps')
    sem_ed = get_semantic_ed(predictions, fixations_dict, max_len, '/zjh/data/FastGaze-cocosearch/semantic_seq_full/stuffthing_maps')
    # sem_ed_t = get_semantic_ed_time(predictions, fixations_dict, max_len, '/zjh/data/Gazeformer-CVPR-2023/SemSS/stuffthing_maps')
...
def generate_and_save_saliency(pred_x, pred_y, tgt_x, tgt_y, img_size, kernel_size=15, sigma=3.0, save_dir="saliency_maps/"):
    ...
    save_saliency_maps(saliency_pred, saliency_tgt, filename_pred=save_dir + "saliency_pred_14.png", filename_tgt=save_dir + "saliency_tgt_14.png")
...
def proximity_loss(pred_x, pred_y, mask=None, threshold=10.0, epsilon=1e-6):
    ...
    for b in range(batch_size):
        ...
        sample_loss = -close_dists.sum() if close_dists.numel() > 0 else 0
        total_loss += sample_loss
        valid_samples += 1
...
def train(epoch, args, model, SlowOpt, MidOpt, FastOpt, loss_fn_token, loss_fn_y, loss_fn_x, loss_fn_t, train_dataloader, model_dir, model_name, device = 'cuda:0', im_h=20, im_w=32, patch_size=16):
    ...
    with tqdm(train_dataloader, unit="batch") as tepoch:
        ...
        for batch_imgs, batch_tgt, batch_tgt_padding_mask, batch_tasks, batch_firstfix in tepoch:
            out_token, out_y, out_x, out_t = model(src = batch_imgs, tgt = batch_firstfix, task = batch_tasks * 0) # Freeview task变为0
            ...
            # disp_loss = dispersion_loss(out_x, out_y, mask=fixation_mask)
            if epoch >= 190: # 最后10轮用距离损失
                disp_loss = dispersion_loss(out_x, out_y, mask=fixation_mask, y_weight=1.1)
                print(token_loss, reg_loss, disp_loss)
                loss = token_loss + reg_loss + t_loss + disp_loss * 1000
            else:
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs+1):
        ...
        args.zerogaze = False
        args.num_samples = 10
        seq_score, sem_seq_score, ed, sem_ed = test(args,model)
        ...
        output_str2 = f"Epoch: {epoch}, best_epo: {best_epo}, bestss: {best_seq:.3f}, seq_score: {seq_score:.3f}, sem_seq_score: {sem_seq_score:.3f}, ed: {ed:.3f}, sem_ed: {sem_ed:.3f}\n"
